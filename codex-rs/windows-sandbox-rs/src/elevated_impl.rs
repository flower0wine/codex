...
mod windows_impl {
    ...
    fn runner_launch_error_requires_reprovision(err: &anyhow::Error) -> bool {
        let msg = err.to_string();
        msg.contains("CreateProcessWithLogonW failed: 1326")
            || msg.contains("CreateProcessWithLogonW failed: 1909")
    }
    ...
    pub fn run_windows_sandbox_capture(
        ...
        (|| -> Result<CaptureResult> {
            ...
            let mut retried_after_credential_failure = false;
            let mut transport = loop {
                match spawn_runner_transport(codex_home, cwd, &sandbox_creds, logs_base_dir) {
                    Ok(transport) => break transport,
                    Err(err)
                        if !retried_after_credential_failure
                            && runner_launch_error_requires_reprovision(&err) =>
                    {
                        log_note(
                            &format!(
                                "runner launch credentials rejected ({err:#}); forcing sandbox user reprovision and retry"
                            ),
                            logs_base_dir,
                        );
                        run_elevated_setup(
